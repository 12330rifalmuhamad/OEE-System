...
function getProductSizeInKg(name) {
  if (!name) return 0.8;
...
async function recalculateOkpLogOee(okpLogId, prismaTx = null) {
  ...
  if (log.totalInput !== null) {
    const sizeInKg = getProductSizeInKg(log.product.name || log.product.size || "");
...
async function startOeeRealTimeTicker() {
  ...
  setInterval(async () => {
    try {
      ...
      console.error("[OEE Ticker] Error in periodic OEE recalculation:", err);
    }
