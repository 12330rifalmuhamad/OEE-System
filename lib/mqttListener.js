...
const { recordHeartbeat, setMachineState, getMachineState } = require("./telemetryCache");
            ...
            statusProcessingQueue = statusProcessingQueue.then(async () => {
              try {
                ...
                if (config.machine.lineProcessId) {
                  ...
                  const stoppedFilling = fillingConfigs.filter(c => {
                    const st = machineStates[c.machineId] || getMachineState(c.machineId);
                    return st !== "RUN";
                  });
                  ...
                  if (stoppedNonFilling.length > 0) {
                    ...
                    if (currentMachineStopped && !isRun) {
                      ...
                      const repConfig = stoppedFilling[0] || fillingConfigs[0];
                      activeDowntimeCode = repConfig ? machineStates[repConfig.machineId] : "STOP";
                      activeMachineId = repConfig ? repConfig.machineId : config.machineId;
                      activeMachineName = repConfig ? repConfig.machine.name : config.machine.name;
                    }
                  } else {
                    overallState = "RUN";
                  }
