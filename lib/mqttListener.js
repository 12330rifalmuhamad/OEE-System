      ...
      client.on("message", async (topic, message) => {
        ...
        try {
          ...
          if (config.counterTopic && topic === config.counterTopic) {
            ...
            if (config.machineId === activeOkp.machineId) {
              ...
              console.log(`[MQTT] OKP: ${activeOkp.okpNumber} | Mesin: ${config.machine.name} | Counter +${qty} pcs (Primary Counter)`);
            } else {
            ...
            statusProcessingQueue = statusProcessingQueue.then(async () => {
              try {
                ...
                if (config.machine.lineProcessId) {
                  ...
                  const stoppedConfig = lineConfigs.find(c => machineStates[c.machineId] && machineStates[c.machineId] !== "RUN");

                  if (stoppedConfig) {
                    overallState = "STOP";
                    activeDowntimeCode = machineStates[stoppedConfig.machineId];
                  }
                } else {
                ...
                if (overallState === "RUN") {
                  ...
                  if (targetCode) {
                    ...
                    console.log(`[MQTT] [DOWNTIME] Line/Machine transitioning to STOP [${targetCode.code}]. Triggered by: ${config.machine.name}`);
